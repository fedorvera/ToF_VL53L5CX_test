...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define POLLING_PERIOD (50U) /* milliseconds */
#define TOF_TASK_STACK_SIZE 1024
/* USER CODE END PD */
...
static osThreadId tofTaskId;
static StackType_t tofTaskBuffer[TOF_TASK_STACK_SIZE];
static osStaticThreadDef_t tofTaskControlBlock;
/* USER CODE BEGIN PV */
static int32_t status = 0;
static RANGING_SENSOR_Capabilities_t Cap;
static RANGING_SENSOR_ProfileConfig_t Profile;
static RANGING_SENSOR_Result_t Result;
volatile static uint32_t dist[4][4];
/* USER CODE END PV */
...
static void Tof_Task(void const * argument);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  /* add threads, ... */
  osThreadStaticDef(tofTask, Tof_Task, osPriorityHigh, 0, TOF_TASK_STACK_SIZE, tofTaskBuffer, &tofTaskControlBlock);
  tofTaskId=osThreadCreate(osThread(tofTask), NULL);
...
static void MX_GPIO_Init(void)
{
  ...
  /**/
  LL_GPIO_ResetOutputPin(I2C_RST_GPIO_Port, I2C_RST_Pin);

  /**/
  LL_GPIO_SetOutputPin(PWREN_GPIO_Port, PWREN_Pin);

  /**/
  LL_GPIO_SetOutputPin(LPn_GPIO_Port, LPn_Pin);
  ...
  GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
  LL_GPIO_Init(I2C_RST_GPIO_Port, &GPIO_InitStruct);
  ...
  GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
  LL_GPIO_Init(PWREN_GPIO_Port, &GPIO_InitStruct);
  ...
  GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
  LL_GPIO_Init(LPn_GPIO_Port, &GPIO_InitStruct);
...
static void MX_VL53L5CX_SimpleRanging_Process(void)
{
  ...
  while (1)
  {
    ...
    for(int i=0;i<4;i++)
    {
        for(int j=0;j<4;j++)
        {
            dist[i][j]=Result.ZoneResult[i*4+j].Distance[0];
        }
    }
